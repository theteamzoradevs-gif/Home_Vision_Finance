import {
  Bell,
  Briefcase,
  Building,
  Building2,
  Check,
  ChevronRight,
  CircleDollarSign,
  CirclePlus,
  Clock,
  FileCheck,
  FileText,
  Handshake,
  Home,
  Landmark,
  Mail,
  MapPin,
  Menu,
  Percent,
  Phone,
  PhoneCall,
  PiggyBank,
  Repeat2,
  Search,
  ShieldCheck,
  Star,
  Timer,
  TrendingUp,
  Wallet,
  X,
  type LucideIcon,
} from "lucide-react";
import { FaInstagram, FaLinkedin, FaYoutube } from "react-icons/fa";

type IconSize = 16 | 18 | 20 | 22 | 24;

function icon(Icon: LucideIcon, size: IconSize = 20) {
  return <Icon size={size} strokeWidth={1.75} aria-hidden />;
}

function brandIcon(Icon: typeof FaInstagram, size: IconSize = 20) {
  return <Icon size={size} aria-hidden />;
}

export function Logo({ size = 40 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 48 48" fill="none" aria-hidden>
      <rect width="48" height="48" rx="12" fill="url(#logoGrad)" />
      <path d="M24 10L10 22h4v14h8v-8h4v8h8V22h4L24 10z" fill="white" fillOpacity="0.95" />
      <defs>
        <linearGradient id="logoGrad" x1="0" y1="0" x2="48" y2="48">
          <stop stopColor="#1a4f9e" />
          <stop offset="1" stopColor="#0f2f5e" />
        </linearGradient>
      </defs>
    </svg>
  );
}

export const Icons = {
  phone: icon(Phone),
  phoneCall: icon(PhoneCall),
  wa: (
    <svg width={24} height={24} viewBox="0 0 24 24" fill="currentColor" aria-hidden>
      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
    </svg>
  ),
  check: icon(Check, 18),
  chevronRight: icon(ChevronRight, 16),
  home: icon(Home, 22),
  shield: icon(ShieldCheck, 20),
  building: icon(Building2, 22),
  transfer: icon(Repeat2, 22),
  rupee: icon(Building2, 22),
  trendingUp: icon(TrendingUp, 22),
  timer: icon(Timer, 22),
  circlePlus: icon(CirclePlus, 22),
  doc: icon(FileText, 22),
  star: <Star size={16} strokeWidth={1.75} className="fill-current" aria-hidden />,
  menu: icon(Menu, 24),
  close: icon(X, 24),
  percent: icon(Percent, 22),
  handshake: icon(Handshake, 22),
  search: icon(Search, 20),
  mail: icon(Mail, 20),
  map: icon(MapPin, 20),
  landmark: icon(Landmark, 22),
  piggyBank: icon(PiggyBank, 22),
  circleDollar: icon(CircleDollarSign, 22),
  instagram: brandIcon(FaInstagram, 18),
  linkedin: brandIcon(FaLinkedin, 18),
  youtube: brandIcon(FaYoutube, 18),
  wallet: icon(Wallet, 22),
  fileCheck: icon(FileCheck, 20),
  bell: icon(Clock, 22),
  hammer: icon(Building, 22),
  briefcase: icon(Briefcase, 22),
  /** @deprecated Use trendingUp instead */
  zap: icon(TrendingUp, 22),
};
